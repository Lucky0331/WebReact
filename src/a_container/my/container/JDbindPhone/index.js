...
class Register extends React.Component {
          ...
          new Promise((resolve, rej) => {
            this.props.actions.bindPhone(params).then(res => {
              if (res.status === 200) {
                ...
                const datasIndex=location.href.indexOf('datas=');
                const datas=location.href.substr(datasIndex + 6);
                this.props.history.push({pathname:'/shop/JDCOM',search:`datas=${datas}`});
              } else {
  ...
  render() {
    ...
    return (
      <div
        className="flex-auto page-box page-bindphone"
          ...
          <div className="logo-info">
            {t ? (
              <span>
                当前绑定手机号
                <br />
                <b style={{ fontSize: ".5rem" }}>{u.mobile}</b>
              </span>
            ) : (
              <span>
                绑定会让您的账号更加安全
...
export default connect(
  ...
  dispatch => ({
    actions: bindActionCreators(
      { getVerifyCode, getUserInfo, updateUserInfo, bindPhone },
      dispatch
    )
  })
)(Register);
